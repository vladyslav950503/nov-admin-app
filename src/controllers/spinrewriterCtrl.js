...
const { spinrewriterLog } = require('../services/logger');
    ...
    try {
        ...
        await Promise.all([
            ...
            if (page.url() == 'https://www.spinrewriter.com/cp-home') {
                ...
                if (await credentialModel.findOne({ type: "spinrewriter" })) {
                    await credentialModel.findOneAndUpdate({ type: "spinrewriter" }, {
                        username: email,
                        password: password
                    });
                } else {
                ...
                await browser.close(true);
                spinrewriterLog.error(`Start session with ${email} failed.`);
                res.status(500).send('Credential is incorrect.');
            }
        });
    } catch (err) {
        spinrewriterLog.error(`Start session with ${email} failed: ${get(err, 'response.data.message') || err.toString()}`);
